...
describe("Upgrade Tests", function () {
  ...
  describe("Beacon Proxy Pattern - OrionTransparentVault", function () {
    ...
    beforeEach(async function () {
      ...
      vaultBeacon = deployed.transparentVaultBeacon;
      ...
      const tx2 = await vaultFactory
        .connect(owner)
        .createVault(strategist.address, "Test Vault 2", "TV2", 0, 0, 0, ethers.ZeroAddress, VaultType.Transparent);
  ...
  describe("Factory Upgrade Pattern", function () {
    ...
    it("Should upgrade factory itself via UUPS", async function () {
      ...
      const currentTransparentBeacon = await vaultFactory.transparentVaultBeacon();
      ...
      expect(await upgradedFactory.transparentVaultBeacon()).to.equal(currentTransparentBeacon);
    });
  });
  ...
  describe("Factory Beacon Management", function () {
    ...
    it("Should create vaults with different implementations after setVaultBeacon", async function () {
      ...
      const tx1 = await vaultFactory
        .connect(owner)
        .createVault(strategist.address, "Vault V1", "VV1", 0, 0, 0, ethers.ZeroAddress, VaultType.Transparent);
      const receipt1 = await tx1.wait();
      ...
      await vaultFactory.connect(owner).setTransparentVaultBeacon(await newBeacon.getAddress());

      // Deploy second vault with V2 implementation
      const tx2 = await vaultFactory
        .connect(owner)
        .createVault(strategist.address, "Vault V2", "VV2", 0, 0, 0, ethers.ZeroAddress, VaultType.Transparent);
      const receipt2 = await tx2.wait();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    it("Should create vaults with same new implementation after vaultBeacon.upgradeTo", async function () {
      ...
      const tx1 = await vaultFactory
        .connect(owner)
        .createVault(strategist.address, "Vault 1", "V1", 0, 0, 0, ethers.ZeroAddress, VaultType.Transparent);
      const receipt1 = await tx1.wait();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const vault1Address = (receipt1?.logs.find((log: any) => log.fragment?.name === "OrionVaultCreated") as any)
        ?.args?.[0];
      ...
      const tx2 = await vaultFactory
        .connect(owner)
        .createVault(strategist.address, "Vault 2", "V2", 0, 0, 0, ethers.ZeroAddress, VaultType.Transparent);
      const receipt2 = await tx2.wait();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const vault2Address = (receipt2?.logs.find((log: any) => log.fragment?.name === "OrionVaultCreated") as any)
        ?.args?.[0];
    ...
    it("Should maintain factory functionality after UUPS upgrade with beacon changes", async function () {
      ...
      await upgradedFactory.connect(owner).setTransparentVaultBeacon(await newBeacon.getAddress());
  ...
  describe("Direct upgradeToAndCall", function () {
    ...
    it("Should cover VaultFactory._authorizeUpgrade via direct upgradeToAndCall", async function () {
      const VaultFactoryFactory = await ethers.getContractFactory("VaultFactory");
      const newImpl = await VaultFactoryFactory.deploy();
      await newImpl.waitForDeployment();

      // Call upgradeToAndCall directly, executing _authorizeUpgrade
      await vaultFactory.connect(owner).upgradeToAndCall(await newImpl.getAddress(), "0x");

      expect(await vaultFactory.owner()).to.equal(owner.address);
    });

    it("Should cover VaultFactory._authorizeUpgrade revert on non-owner", async function () {
      const VaultFactoryFactory = await ethers.getContractFactory("VaultFactory");
      const newImpl = await VaultFactoryFactory.deploy();
      await newImpl.waitForDeployment();

      await expect(
        vaultFactory.connect(user).upgradeToAndCall(await newImpl.getAddress(), "0x"),
