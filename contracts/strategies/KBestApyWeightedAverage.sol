...
/**
 * @title KBestApyWeightedAverage
 * @notice Selects the top-K assets by estimated APY and allocates weight proportionally to APY.
 *         Falls back to equal weight when all selected assets have APY = 0.
 * @author Orion Finance
 * @dev Call updateCheckpoints() before submitIntent() to refresh APY data.
 * @custom:security-contact security@orionfinance.ai
 */
contract KBestApyWeightedAverage is ApyStrategistBase {
    ...
    /// @dev Weights proportional to APY. Falls back to equal weight when totalApy == 0.
