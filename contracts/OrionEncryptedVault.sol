...
//         uint32[] memory finalAmounts = new uint32[](config.whitelistedAssetsLength());
...
//             uint256 index = config.whitelistedAssetIndex(token);
...
//         if (order.length == 0) revert OrderIntentCannotBeEmpty();
//         euint32[] memory finalAmounts = new euint32[](config.whitelistedAssetsLength());
